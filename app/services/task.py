...
2. check if the network is available. If you are in China, it is recommended to use a VPN and enable the global traffic mode.
        ...
        if tts_error:
            logger.error(f"tts internal error: {tts_error}")
...
def start(task_id, params: VideoParams, stop_at: str = "video"):
    ...
    # Improve relevance for single-video generation by defaulting to semantic alignment.
    if (
        params.video_source != "local"
        and params.video_count == 1
        and params.video_concat_mode == VideoConcatMode.random
    ):
        logger.info(
            "Auto-switching video concat mode from random to semantic for better script relevance"
        )
        params.video_concat_mode = VideoConcatMode.semantic
