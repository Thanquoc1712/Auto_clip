...
def generate_video(
    ...
    def create_text_clip(subtitle_item):
        ...
        # Repair common UTF-8 mojibake patterns (e.g. "tiÐng", "khÃ´ng")
        # that can appear when subtitle text is decoded with a wrong codec upstream.
        def repair_mojibake(text: str) -> str:
            if not text:
                return text
            bad_markers = ("Ã", "Ð", "Ä", "á»", "áº")
            if not any(m in text for m in bad_markers):
                return unicodedata.normalize("NFC", text)
            try:
                repaired = text.encode("latin-1", errors="ignore").decode("utf-8", errors="ignore")
                # Keep repaired value only when it looks plausibly better.
        ...
        # Render subtitle with PIL instead of TextClip to avoid Unicode mojibake
        # on some Windows text backends.
        try:
            pil_font = ImageFont.truetype(font_path, params.font_size)
        except Exception:
            pil_font = ImageFont.load_default()

        canvas_w = int(max_width)
        canvas_h = int(max(1, txt_height + params.font_size * 0.8))
        img = Image.new("RGBA", (canvas_w, canvas_h), (0, 0, 0, 0))
        draw = ImageDraw.Draw(img)

        # Optional subtitle background for readability.
        if params.text_background_color:
            draw.rectangle([(0, 0), (canvas_w, canvas_h)], fill=(0, 0, 0, 90))
        ...
        text_fill = _hex_to_rgba(params.text_fore_color, 255)
        stroke_fill = _hex_to_rgba(params.stroke_color, 255)
        stroke_w = max(0, int(params.stroke_width))

        lines = wrapped_txt.split("\n")
        line_height = int(params.font_size * 1.25)
        total_h = line_height * len(lines)
        y = max(0, (canvas_h - total_h) // 2)

        for line in lines:
            bbox = draw.textbbox((0, 0), line, font=pil_font, stroke_width=stroke_w)
    ...
    def parse_srt_utf8(srt_path: str):
        """Parse SRT with strict UTF-8 to avoid mojibake on Vietnamese text."""
        ...
        for block in blocks:
            ...
            if not text_value:
                continue
            items.append(((start, end), text_value))
        return items
    ...
    if subtitle_path and os.path.exists(subtitle_path):
        ...
        if use_word_highlighting:
            ...
            # Traditional subtitle rendering (parse UTF-8 ourselves to preserve Vietnamese text)
            text_clips = []
            for item in parse_srt_utf8(subtitle_path):
