...
def ensure_utf8_console():
    # Prevent UnicodeEncodeError on Windows consoles using legacy codepages.
...
with middle_panel:
    ...
    with st.container(border=True):
        ...
        if (
            str(getattr(params, "video_language", "")).lower().startswith("vi")
            and selected_tts_server == "chatterbox"
        ):
            speech_key = config.azure.get("speech_key", "").strip()
            speech_region = config.azure.get("speech_region", "").strip()
            if speech_key and speech_region:
                st.warning(
                    "Du an tieng Viet: da tu chuyen sang Azure TTS V2 "
                    "de su dung giong vi-VN chuan hon."
                )
                selected_tts_server = "azure-tts-v2"
            else:
                st.warning(
                    "Ban dang chon Chatterbox cho tieng Viet. Giong co the chua chuan. "
                    "Neu muon giong vi-VN chuan, hay cau hinh Azure Speech Key/Region "
                    "va chon Azure TTS V2."
                )
        ...
        if (
            ...
            and str(getattr(params, "video_language", "")).lower().startswith("vi")
        ):
            filtered_voices = sorted(
                filtered_voices,
                key=lambda x: (0 if x.lower().startswith("vi-") else 1, x),
            )
